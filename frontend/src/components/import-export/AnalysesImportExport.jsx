...
const AnalysesImportExport = ({ analysisId, onImportSuccess, showImport = true, showExport = true }) => {
  ...
  // Funkcja do importowania analizy
  const handleImport = async () => {
    ...
    try {
      ...
      // Endpoint dla analiz
      const endpoint = '/analyses/import';
      ...
      showSnackbar('Analiza została pomyślnie zaimportowana!', 'success');

      // Informujemy komponent nadrzędny o potrzebie odświeżenia listy analiz
      if (onImportSuccess) {
        setTimeout(() => {
          onImportSuccess();
          handleCloseImport();
      ...
      if (error.code === 'ECONNABORTED' || !error.response) {
        ...
        errorMessage = error.response?.data?.error || 'Wystąpił błąd podczas importu analizy';
      }
  ...
  // Funkcja do eksportowania analizy
  const handleExport = async () => {
    if (!analysisId) {
      showSnackbar('Nie można eksportować analizy. Brak ID analizy.', 'error');
      return;
    }
    ...
    try {
      // Endpoint dla eksportu analizy
      const endpoint = `/analyses/${analysisId}/export`;
      ...
      // Prefiks nazwy pliku dla analizy
      ...
      showSnackbar('Analiza została pomyślnie wyeksportowana!', 'success');
      ...
      const errorMessage = error.response?.data?.error || 'Wystąpił błąd podczas eksportu analizy';
      showSnackbar(errorMessage, 'error');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      <Dialog open={importDialog} onClose={handleCloseImport} maxWidth="sm" fullWidth>
        <DialogTitle>Import analizy</DialogTitle>
        <DialogContent>
          {error && (
          ...
          <Box sx={{ mb: 3, mt: 2 }}>
            <Typography variant="body2" color="text.secondary" gutterBottom>
              Wybierz plik analizy w formacie JSON, XML lub YAML do zaimportowania.
          ...
          <Box sx={{ mb: 2, mt: 2 }}>
            <Typography variant="body2" color="text.secondary" gutterBottom>
              Co zrobić gdy analiza o takiej nazwie już istnieje?
            </Typography>
            ...
            >
              <FormControlLabel
                value="prefix"
                control={<Radio />}
                label="Dodaj prefix do nazwy (np. 'Kopia - Moja Analiza')"
                disabled={loading}
              />
              <FormControlLabel
                value="reject"
              ...
              <FormControlLabel
                value="replace"
                control={<Radio />}
                label="Zastąp istniejącą analizę"
                disabled={loading}
              />
            </RadioGroup>
          </Box>
      ...
      <Dialog open={exportDialog} onClose={handleCloseExport} maxWidth="sm" fullWidth>
        <DialogTitle>Eksport analizy</DialogTitle>
        <DialogContent>
          {error && (
            <Alert severity="error" sx={{ mb: 2 }}>
              <AlertTitle>Błąd</AlertTitle>
          ...
          <Box sx={{ mb: 3, mt: 2 }}>
            <Typography variant="body2" color="text.secondary" gutterBottom>
              Wybierz format, w którym chcesz wyeksportować analizę.
            </Typography>
          </Box>
...
export default AnalysesImportExport;