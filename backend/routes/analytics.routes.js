...
/**
 * Zwraca listę dostępnych krajów z obu API (WHO i World Bank)
 */
...
/**
 * Zwraca dostępne lata dla danego kraju i typu analizy z obu API (WHO i World Bank)
 */
router.get('/analytics/available-years', async (req, res) => {
 ...
 */
router.get('/analytics/analysis-types', (req, res) => {
  const analysisTypes = [
    {
 ...
 */
router.put('/analyses/:id', authenticateToken, async (req, res) => {
  const analysisId = req.params.id;
  const { name } = req.body;
